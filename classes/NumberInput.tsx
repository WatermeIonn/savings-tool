...
export class NumberInput extends Input {
  public validate(): boolean {
    if (this.value) {
      ...
      if (this.validationRules?.min !== undefined && Number(this.value) < this.validationRules.min) {
        this.hasError = true;
        this.errorMessage = `Value must be higher than ${this.validationRules.min}`;
        return false;
      }

      if (this.validationRules?.max !== undefined && Number(this.value) > this.validationRules.max) {
        this.hasError = true;
